...
export async function renewDocument(
  ...
  if (!values.expiry_date) return { ok: false, error: "Expiry date is required" };
  if (values.issue_date && values.issue_date > values.expiry_date)
  ...
  const { data, error } = await supabase.rpc("renew_document", {
    ...
    p_expiry_date: values.expiry_date,
